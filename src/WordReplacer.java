        ...
        while (check(str)){
            for(int i=0;i<constnames.length;i++){
                if(i<=6){
                    ...
                    str.replaceFirst(constnames[i],sc.getVal());
                }
                else{
                    switch (i){
                        case 7:
                            str.replaceFirst(constnames[i],stc.makeSentence(null));
                            break;
                        case 8:
                            if(env==null) break;
                            str.replaceFirst(constnames[i],wt.getEnv(env));
                            break;
                        case 9:
                            if(chara==null) break;
                            int replaced=pro.anInt(chara.size());
                            str.replaceFirst(constnames[i],chara.get(replaced));
